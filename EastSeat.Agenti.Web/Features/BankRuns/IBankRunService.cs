...
public interface IBankRunService
{
    ...
    /// the agent's cash wallet to the target bank wallet (identified by wallet type) and updates
    /// both balances.
    /// </summary>
    ...
    /// Used to display the cash wallet balance in the UI.
    /// </summary>
    Task<List<AgentWalletDto>> GetAgentWalletsAsync(string userId, WalletTypeEnum walletType);

    /// <summary>
    /// Returns the wallet types (excluding Cash) for which the agent has an active wallet.
    /// Used to populate the destination wallet-type selector when recording a bank run.
    /// </summary>
    Task<List<WalletTypeOptionDto>> GetAgentNonCashWalletTypesAsync(string userId);
}
